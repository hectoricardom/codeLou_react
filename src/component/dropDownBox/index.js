...
class HRMDropDown extends Component {
  ...
  render() { 
     const {title,list,toshow} = this.props;
     const {index} = this.state;
     var valid = index?list[index]?list[index][toshow]?true:false:false:false;     
      return (                         
              <div className={`drop-box-container mqn2-be2 ${this.state.active?"active":""} ${valid?"selected":""} ${this.state.tab_visible?'dp-open':''}`}>
                <div className="drop-box-header">
                  ...
                  <div className="center-label">
                    <span onClick={this.OpenTabView.bind(this)}>{index?list[index]?list[index][toshow]:title:title}</span>
